...
- [x] 11. **[HUMAN]** Once verification passes: in the Cloudflare dashboard, connect the Worker to this GitHub repo (Workers & Pages → the `meritly` Worker → Settings → Builds), set the production branch to `master` so Workers Builds deploys automatically on future pushes. Since preview builds are public by default, consider disabling them or gating with Cloudflare Access once the app carries real employee/compensation data — not urgent today since only the auth skeleton is live. _(Done 2026-09-20 — GitHub repo connected via Workers Builds; `master` set as the production branch. Preview-build lockdown deliberately deferred, tracked below as a follow-up once real employee/compensation data lands.)_
- [x] 12. **[AGENT]** Update `context/changes/deployment/deployment-plan.md` with the actual outcome: exact commands run, production Supabase project reference (URL only, never keys), secret names set (not values), deployed URL, smoke-test result, and the Workers Builds GitHub-connection state. _(This edit.)_
...
**Complete — deployed, wired to production Supabase, end-to-end auth confirmed, and auto-deploy wired up.** All 12 steps done. The app is live at **https://meritly.meritly.workers.dev**, sign-up/sign-in/sign-out work end-to-end against the real production Supabase project, and the Worker is now connected to GitHub via Workers Builds (production branch `master`) so future pushes deploy automatically without a manual `wrangler deploy`.

**Open follow-ups (not blockers for this deploy):**

1. Preview-build lockdown (Cloudflare Access or disabling preview URLs) — deferred until the app carries real employee/compensation data, per step 11's note.
2. The smoke-script `@example.com` issue from step 10 — `scripts/smoke.mjs` needs a Supabase-accepted test domain, or this stays a local-Supabase-only check per CLAUDE.md.
